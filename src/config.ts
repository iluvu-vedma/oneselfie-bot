...
/** С запасом под лимит Telegram на входящее сообщение (см. TEXT_LIMIT). */
export const MAX_PROMPT_LEN = 4000;
/**
 * Лимит текстового сообщения Telegram. Описание кадра уходит отдельным
 * сообщением; `clip()` в prompt.ts режет его по границе слова, если промпт
 * вместе с обёрткой в лимит не помещается — например, escape раздувает
 * «&» и «<» в разы.
 */
...
/**
 * TTL записи задачи в KV. Двое суток, а не час: пока запись жива, задачу можно
 * добрать и вернуть за неё искры, а с её смертью возврат становится невозможен
 * навсегда. Крон на бесплатном Vercel ходит раз в сутки — значит запись обязана
 * пережить как минимум два его прихода, иначе потерянный коллбэк у человека,
 * который в бот больше не вернулся, означает молча съеденные искры.
 */
export const TASK_TTL_SEC = 48 * 60 * 60;
...
// ── Устойчивость ─────────────────────────────────────────────────────────────
/**
 * Сколько ждёт вебхук, прежде чем ответить Telegram и бросить работу.
 *
 * У grammY по умолчанию 10 секунд и `onTimeout: "throw"`: превышение отдаёт
 * Telegram пятисотку, тот присылает апдейт заново — и загрузка селфи стартует
 * второй раз. Ставим чуть меньше `maxDuration` функции (30 с) и отвечаем
 * двухсоткой: повторять апдейт не за чем, недоделанное подберёт добор.
 */
export const WEBHOOK_TIMEOUT_MS = 25_000;

/**
 * Сколько ждать освобождения чата. Альбом из четырёх селфи — четыре
 * параллельных запуска, и без очереди человек получает четыре экрана вместо
 * одного. Заливка одного селфи занимает секунду-две, четвёртому ждать втрое
 * дольше — восьми секунд хватает с запасом.
 */
export const CHAT_LOCK_WAIT_MS = 8_000;
/** Как часто проверять, освободился ли чат. */
export const CHAT_LOCK_POLL_MS = 120;
/**
 * Срок замка чата. Заведомо больше самого долгого обработчика, но меньше
 * времени жизни функции: процесс могут убить, не дав снять замок, и тогда
 * человек не должен остаться запертым дольше одного апдейта.
 */
export const CHAT_LOCK_TTL_SEC = 30;

/** Потолок ожидания ответа kie. Без него зависшее соединение съедает всю функцию. */
export const KIE_TIMEOUT_MS = 20_000;

/**
 * Сколько раз повторять запрос к Telegram при 429 и 5xx. Telegram сам говорит,
 * сколько ждать (`retry_after`), поэтому пауза берётся из ответа, а не из головы.
 */
export const TG_RETRIES = 3;
/** Дольше этого не ждём даже по просьбе Telegram: функция не бесконечная. */
export const TG_MAX_WAIT_MS = 8_000;

/** Сколько задач разбирает один проход крона. */
export const SWEEP_BATCH = 100;
/** Сколько задач разбирается одновременно. Лимит kie — 20 запросов на 10 секунд. */
...
/**
 * Сколько последних ошибок хранится своим кольцом. Логи Vercel на бесплатном
 * тарифе живут час, а вопрос «что вчера ломалось» задают на следующий день.
 */
export const ERRORS_KEEP = 50;
/** Сколько ошибок показывает экран здоровья. */
export const HEALTH_ERRORS = 4;
